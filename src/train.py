...
def main():
    ...
    for name, clf in get_models().items():
        with mlflow.start_run(run_name=name):
            ...
            # MLflow 3.x defaults to skops serialization, which refuses to write
            # sklearn internals like _RemainderColsList (emitted by some sklearn
            # versions inside ColumnTransformer). cloudpickle handles the whole
            # pipeline and is what joblib.dump uses below anyway.
